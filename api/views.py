...
from rest_framework.permissions import IsAuthenticated, DjangoModelPermissions # Changed IsAdminUser to DjangoModelPermissions

from payroll import models


class CreateEmployeeView(generics.CreateAPIView):
    queryset = models.EmployeeProfile.objects.all() # Used by DjangoModelPermissions to infer model
...
class PaydayView(viewsets.ViewSet):
    queryset = models.Payday.objects.none() # Added for DjangoModelPermissions
    permission_classes = [DjangoModelPermissions] # Changed from IsAdminUser

    def list(self, request):
        pay = models.Payday.objects.all() # Actual data queryset
    ...
    def retrieve(self, request, pk=None):
        ...
        queryset_data = models.Payday.objects.filter(paydays_id=pk) # Original filtering for retrieve
        serializer = PayDaySerializer(queryset_data, many=True) # many=True seems odd for a retrieve on pk. Usually one object.
                                                            # If paydays_id can map to multiple Payday entries, then it's a list.
        return Response(serializer.data, status=status.HTTP_200_OK)
