...
)  # To setup groups and permissions
...
class APIPermissionTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.hr_user = User.objects.create_user(
            email="hruser@example.com", password="password123", first_name="HR"
        )
        ...
        cls.regular_user = User.objects.create_user(
            email="user@example.com", password="password123", first_name="Reg"
        )
        cls.regular_user_profile = EmployeeProfile.objects.create(
        ...
        try:
            view_ep_perm = Permission.objects.get(
                codename="view_employeeprofile", content_type=ep_ct
            )
            cls.hr_group.permissions.add(view_ep_perm)
        except Permission.DoesNotExist:
            print(
                "Warning: 'view_employeeprofile' permission not found during test setup."
            )
        ...
        try:
            add_ep_perm = Permission.objects.get(
                codename="add_employeeprofile", content_type=ep_ct
            )
            cls.hr_group.permissions.add(add_ep_perm)  # Give HR add_employeeprofile
        except Permission.DoesNotExist:
            print(
                "Warning: 'add_employeeprofile' permission not found during test setup."
            )
        ...
        if not cls.admin_user.groups.filter(
            name="Admin"
        ).exists():  # Add admin_user to Admin group if not already
            cls.admin_user.groups.add(admin_group)

        try:
            add_ep_perm_admin = Permission.objects.get(
                codename="add_employeeprofile", content_type=ep_ct
            )
            admin_group.permissions.add(add_ep_perm_admin)
            user_ct = ContentType.objects.get_for_model(
                User
            )  # Django User or CustomUser
            add_user_perm = Permission.objects.get(
                codename="add_user", content_type=user_ct
            )  # or add_customuser
    ...
    def test_create_employee_hr_user_denied_if_no_user_add_perm(self):
        ...
        hr_has_add_user_perm = self.hr_user.has_perm(
            f"{user_model_ct.app_label}.add_{user_model_ct.model}"
        )
        ...
        if not hr_has_add_user_perm:
            ...
            data = {
                "user": {
                    "email": "hr_creates_this@example.com",
                    "password": "password123",
                    "first_name": "HR",
                    "last_name": "Created",
                },
            ...
            response = self.client.post(
                reverse("api:employee-create"), data, format="json"
            )
    ...
    def test_create_employee_admin_user_allowed(self):
        ...
        data = {
            "user": {
                "email": "admin_creates_this@example.com",
                "password": "password123",
                "first_name": "Admin",
                "last_name": "Created",
            },
        ...
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            print(
                "Admin create employee failed due to bad request data:", response.data
            )
    ...
    def test_list_employee_self_unauthenticated(self):
        ...
        )  # IsAuthenticated is used

    # TODO: Add tests for ListPayrollView and CreatePayrollView (similar to Employee views)
    # TODO: Add tests for PayrollRunEntryView (ViewSet, list and retrieve actions)
