...
def index(request):
    ...
    emp = models.EmployeeProfile.emp_objects.all() # Consider if this should be permission limited
    count = emp.count()
...
def dashboard(request):
    ...
    elif request.user.has_perm('payroll.view_employeeprofile'): # Representative permission for HR
...
@permission_required('payroll.view_employeeprofile', raise_exception=True) # Example permission for HR dashboard
def hr_dashboard(request):
    ...
    recent_performance_reviews = models.PerformanceReview.objects.all().order_by("-review_date")[:5]
    department_distribution = models.EmployeeProfile.objects.values("department__name").annotate(count=Count("id"))
    department_labels = json.dumps([item["department__name"] for item in department_distribution])
...
def employee_list(request):
    ...
    employees = models.EmployeeProfile.objects.all() # Further filtering by query params
    if query:
    ...
    return render(request, "employee/employee_list.html", {"employees": employees, "departments": departments})
...
@permission_required(['payroll.add_employeeprofile', 'users.add_customuser'], raise_exception=True)
def add_employee(request):
    if request.method == "POST":
        ...
        if user_form.is_valid() and employee_form.is_valid():
            ...
            employee_profile.first_name = user.first_name # Assuming CustomUser has these fields
...
def update_employee(request, id):
    ...
    if form.is_valid():
        ...
        return redirect("payroll:index") # Consider redirecting to employee list or profile
...
class EmployeeCreateView(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    template_name = "employee/add.html" # This might conflict with the FBV add_employee's template
    success_url = reverse_lazy("payroll:employee_list")
    permission_required = ('payroll.add_employeeprofile', 'users.add_customuser') # Assuming form also handles CustomUser creation
...
class EmployeeUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    model = models.EmployeeProfile
    form_class = EmployeeProfileForm
    template_name = "employee/update.html" # May conflict with FBV update_employee template
    success_url = reverse_lazy("payroll:employee_list")
...
def employee(request, user_id: int): # user_id here is CustomUser.id
    ...
    if request.user.id == user_id or request.user.has_perm('payroll.view_employeeprofile'):
        employee_profile_to_display = target_user_profile
    else:
        raise HttpResponseForbidden("You are not authorized to view this profile.")

    pay = models.Payday.objects.filter(payroll_id__pays__user_id=employee_profile_to_display.user.id)
    context = {"emp": employee_profile_to_display, "pay": pay}
    return render(request, "employee/profile.html", context)


@permission_required('payroll.view_performancereview', raise_exception=True)
def performance_review_list(request): # This view lists all reviews, so general perm is fine
    ...
    return render(request, "reviews/performance_review_list.html", {"reviews": reviews, "form": form})
...
def performance_review_detail(request, review_id):
    ...
    if not (request.user == review.employee.user or request.user.has_perm('payroll.view_performancereview')):
...
def add_performance_review(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(request, "Error adding performance review. Please check the form.")
