...
from payroll.permissions import setup_groups_and_permissions # Import the setup function
...
class EmployeeViewSecurityTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user = User.objects.create_user(email='testuser@example.com', password='password123', first_name='Test', last_name='User')
        ...
        cls.hr_user = User.objects.create_user(email='hruser@example.com', password='password123', first_name='HR', last_name='Person')
        ...
        cls.no_perm_user = User.objects.create_user(email='noperm@example.com', password='password123', first_name='No', last_name='Permission')
        ...
        cls.no_perm_user.groups.add(employee_group) # Ensure they get whatever 'Employee' group gets

        # Create employee profiles for these users
        cls.employee_profile_user = EmployeeProfile.objects.create(user=cls.user, email=cls.user.email, first_name=cls.user.first_name, last_name=cls.user.last_name)
        cls.employee_profile_hr = EmployeeProfile.objects.create(user=cls.hr_user, email=cls.hr_user.email, first_name=cls.hr_user.first_name, last_name=cls.hr_user.last_name)
        cls.employee_profile_noperm = EmployeeProfile.objects.create(user=cls.no_perm_user, email=cls.no_perm_user.email, first_name=cls.no_perm_user.first_name, last_name=cls.no_perm_user.last_name)
    ...
    def test_employee_list_requires_login_and_permission(self):
        ...
        response = self.client.get(reverse('payroll:employee_list'))
        self.assertEqual(response.status_code, 403) # Expect Forbidden due to @permission_required
    ...
    def test_add_employee_fbv_permission_denial_for_hr(self):
        ...
        self.client.login(email='hruser@example.com', password='password123')
        response = self.client.get(reverse('payroll:add_employee')) # Assuming this is the FBV
        self.assertEqual(response.status_code, 403) # Because it needs users.add_customuser too
    ...
    def test_employee_profile_view_own_profile(self):
        ...
        response = self.client.get(reverse('payroll:employee_profile', args=[self.user.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_employee_profile_view_other_profile_denied(self):
        ...
        self.client.login(email='noperm@example.com', password='password123')
        response = self.client.get(reverse('payroll:employee_profile', args=[self.user.id]))
        self.assertEqual(response.status_code, 403) # Denied because no specific view_employeeprofile perm
