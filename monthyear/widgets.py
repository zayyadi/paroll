...
class MonthSelectorWidget(widgets.MultiWidget):
    def __init__(self, attrs=None):
        ...
        month_widget_attrs["class"] = (
            month_widget_attrs.get("class", "")
            + f" {sub_widget_base_class} flex-grow min-w-0 w-monthyear-month"
        ).strip()
        ...
        year_widget_attrs["class"] = (
            year_widget_attrs.get("class", "")
            + f" {sub_widget_base_class} w-24 min-w-0 w-monthyear-year"
        ).strip()
    ...
    def media(self):
        ...
        media.add_css({"screen": (static("monthyear/field/widget_month.css"),)})
        return media
